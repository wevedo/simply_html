...
const lastInteractionStore = new Map();
...
async function main() {
    ...
    zk.ev.on("messages.upsert", async (update) => {
        ...
        const lastInteraction = lastInteractionStore.get(sender) || 0;
        if (Date.now() - lastInteraction < 24 * 60 * 60 * 1000) return;

        lastInteractionStore.set(sender, Date.now());
