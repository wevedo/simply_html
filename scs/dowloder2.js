...
adams({
  ...
  categorie: 'Download',
  reaction: '📽️'
}, async (chatId, messageObj, options) => {
  const { repondre: reply, ms, arg: args } = options;

  if (!args[0]) {
    reply("Insert a public TikTok video link!");
    return;
  }

  const videoUrl = args.join(" ");
  try {
    const response = await fetch(`https://api.ibrahimadams.us.kg/api/download/tiktok?apikey=ibraah-tech&url=${encodeURIComponent(videoUrl)}`);
    const data = await response.json();

    messageObj.sendMessage(chatId, {
      fileName: data[0].nama,
      mimetype: data[0].mime,
      caption: "Downloaded by BWM-MD: " + data[0].nama
    }, {
      quoted: ms
    });

  } catch (error) {
    reply("I am unable to download the file.\n" + error);
  }
});
...
adams({
  ...
  if (!args[0]) {
    reply("Insert a public Facebook video link!");
    return;
  }

  const videoUrl = args.join(" ");
  try {
    const fbInfo = await getFBInfo(videoUrl);

    const caption = `\n *Title:* ${fbInfo.title}\n\n *Direct Link:* ${fbInfo.url}\n `;
    messageObj.sendMessage(chatId, {
      image: { url: fbInfo.thumbnail },
      caption: caption
    }, {
      quoted: ms
    });
    ...
    reply("An error occurred while downloading your media.");
  }
});

// Secondary Facebook Download Handler
adams({
  nomCom: "fb2",
  categorie: "Download",
  reaction: "📽️"
